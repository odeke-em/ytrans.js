...
YTrans.prototype.init = function(init_params, cb) {
    ...
    // Make the api_key unenumerable and not console loggable by default
    if (typeof Object.defineProperty === 'function')
        Object.defineProperty(this, 'api_key', {enumerable: false});
...
YTrans.prototype.set_default_lang = function(lang, cb) {
    if (! (lang && typeof lang === 'string' && lang.length >= 1))
        return this.result_handler(false, 'Cannot only set default_lang to a non empty string', cb);
...
YTrans.prototype.appendKeyToURI = function(obj) {
    ...
    return util.format("%s/%s?key=%s",
              obj.uri || this.translate_uri, obj.method || 'translate', this.api_secret);
};
...
YTrans.prototype.translate = function(data, cb) {
    ...
    if (! data)
        ...
        return this.result_handler(
                    null, 'Expecting an object if a string is not passed in', cb);
    else {
